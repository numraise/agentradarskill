 ...
 * แก้ไขปัญหา Syntax Error (Unexpected "entity") โดยใช้ Universal Syntax
 */
...
namespace agentGuard {
    ...
    /**
     * คำสั่ง Execute ภายใน (Universal Syntax)
     * ทำงานโดยการเช็คที่ตัว mob และส่งคำสั่งมาที่ผู้เล่นที่ใกล้ที่สุด (@p)
     */
    function detectMobs(): void {
        // ใช้รูปแบบ execute @e[...] ~ ~ ~ ซึ่งรองรับทั้ง Parser เก่าและใหม่
        // c=1 เพื่อจำกัดการแจ้งเตือนเพียงข้อความเดียวต่อการสแกน
        player.execute(
            "execute @e[family=monster,r=" + _r + ",c=1] ~ ~ ~ tellraw @p {\"rawtext\":[{\"text\":\"§c[AgentGuard] §fตรวจพบ hostile mob ในระยะ " + _r + " บล็อก!\"}]}"
        )
    }
}
